Вот актуальное описание архитектуры и структуры проекта **Blinkoff IoT Platform** на момент окончания **Недели 3** (Стабилизация, Безопасность, HTTP API).

Мы убрали пока не реализованные части (Kafka, Redis, Android) и сфокусировались на том, что реально работает сейчас.
